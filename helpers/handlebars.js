...
function getLifeHealthDescription(hp) {
  hp = Number(hp);
  if (hp === 100) {
...
hbs.registerHelper('life_health_description', getLifeHealthDescription);
...
Handlebars.registerHelper('get_vendor_property', function log(id, property) {
  if (!game.vendors[id]) {
    return 'Bad vendor';
  }
...
function getObitHeat(awareness) {
  let final = '';
  const countryArr = [];
...
hbs.registerHelper('get_obit_heat', getObitHeat);

function getObitStashFlavor(size) {
...
hbs.registerHelper('get_obit_stash_flavor', getObitStashFlavor);

function getObitMemories(actions) {
  if (actions.length <= 5) {
    return localization('obituary_memories_nothing');
  }
  ...
  return `${localization('obituary_memories_morning', action)}  ${localization(
    `obituary_memories_${action.type}`,
    action
  )}  ${localization('obituary_memories_evening', action)}`;
}

hbs.registerHelper('get_obit_memories', getObitMemories);

function getLocalization(phrase, data) {
  let fullPhrase;
  if (data) {
...
hbs.registerHelper('localization', getLocalization);

module.exports = {
  getLifeHealthDescription: getLifeHealthDescription,
  getObitHeat: getObitHeat,
  getObitStashFlavor: getObitStashFlavor,
  getObitMemories: getObitMemories,
