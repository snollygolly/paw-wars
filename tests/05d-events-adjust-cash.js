...
describe('Events - Simulation Validation (Adjust Cash)', () => {
	...
	it('event should update the current event', function eventSimulation(done) {
		let newCash = Math.round(adjustmentAmount * config.GAME.market.base_price);
		expect(newLife.current.event).to.be.a('string');
		expect(newLife.current.event).to.equal(`This should have an amount here -> ${newCash}`);
		return done();
	});

	it('event should generate cash in the player finance', function eventSimulation(done) {
		// we should have more money now
		let newCash = Math.round(adjustmentAmount * config.GAME.market.base_price) + oldLife.current.finance.cash;
